-- "Windows XP x64" was a platform, which put a processor word inside an operating
-- system's name. A platform is the system; the architecture is a separate axis, and a
-- release or a file can reach more than one of them at once. The DirectX 9.0c packages
-- are the clearest case: one zip carries both the x86 and the x86_64 build.
...
CREATE INDEX version_architectures_by_architecture ON version_architectures (architecture_slug);
CREATE INDEX file_architectures_by_architecture ON file_architectures (architecture_slug);

INSERT INTO version_architectures (version_id, architecture_slug)
SELECT id, architecture_slug FROM versions
WHERE architecture_slug IS NOT NULL AND architecture_slug <> '';

-- a file kept its own architecture where it had one, and otherwise its release's
INSERT INTO file_architectures (file_id, architecture_slug)
SELECT f.id, COALESCE(f.architecture_slug, v.architecture_slug)
FROM files f JOIN versions v ON v.id = f.version_id
WHERE COALESCE(f.architecture_slug, v.architecture_slug) IS NOT NULL;

-- Anything that named the 64-bit edition of XP now names XP and says x64 beside it.
INSERT OR IGNORE INTO file_architectures (file_id, architecture_slug)
SELECT file_id, 'x64' FROM file_platforms WHERE platform_slug = 'windows-xp-x64';

INSERT OR IGNORE INTO version_architectures (version_id, architecture_slug)
SELECT version_id, 'x64' FROM version_platforms WHERE platform_slug = 'windows-xp-x64';

INSERT OR IGNORE INTO file_platforms (file_id, platform_slug)
SELECT file_id, 'windows-xp' FROM file_platforms WHERE platform_slug = 'windows-xp-x64';

INSERT OR IGNORE INTO version_platforms (version_id, platform_slug)
SELECT version_id, 'windows-xp' FROM version_platforms WHERE platform_slug = 'windows-xp-x64';

INSERT OR IGNORE INTO software_platforms (software_slug, platform_slug)
SELECT software_slug, 'windows-xp' FROM software_platforms WHERE platform_slug = 'windows-xp-x64';

DELETE FROM file_platforms WHERE platform_slug = 'windows-xp-x64';
DELETE FROM version_platforms WHERE platform_slug = 'windows-xp-x64';
DELETE FROM software_platforms WHERE platform_slug = 'windows-xp-x64';
DELETE FROM platforms WHERE slug = 'windows-xp-x64';

ALTER TABLE versions DROP COLUMN architecture_slug;
ALTER TABLE files DROP COLUMN architecture_slug;
