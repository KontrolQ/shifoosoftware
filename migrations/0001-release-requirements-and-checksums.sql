...
ALTER TABLE versions ADD COLUMN minimum_cpu_slug TEXT REFERENCES processors (slug) ON DELETE SET NULL;
ALTER TABLE versions ADD COLUMN minimum_cpu_speed INTEGER;
ALTER TABLE versions ADD COLUMN minimum_cpu_speed_unit TEXT;
ALTER TABLE versions ADD COLUMN minimum_ram_size INTEGER;
ALTER TABLE versions ADD COLUMN minimum_ram_unit TEXT;
ALTER TABLE versions ADD COLUMN minimum_disk_size INTEGER;
ALTER TABLE versions ADD COLUMN minimum_disk_unit TEXT;

ALTER TABLE files ADD COLUMN architecture_slug TEXT REFERENCES architectures (slug) ON DELETE SET NULL;
ALTER TABLE files RENAME COLUMN sha256 TO checksum;
ALTER TABLE files ADD COLUMN checksum_algorithm TEXT;

UPDATE files SET checksum_algorithm = CASE length(checksum)
    WHEN 40 THEN 'SHA-1' WHEN 64 THEN 'SHA-256' WHEN 128 THEN 'SHA-512' END
