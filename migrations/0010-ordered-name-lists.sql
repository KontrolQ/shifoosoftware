-- Every list a page prints is in its own running order, not the order the join
-- happened to meet the rows in.
...
SELECT
    ...
    (SELECT group_concat(held, ', ') FROM
        (SELECT p.name AS held FROM software_publishers sp JOIN publishers p ON p.slug = sp.publisher_slug
         WHERE sp.software_slug = s.slug ORDER BY p.sort_order, p.name)) AS publisher_names,
    (SELECT group_concat(held, ', ') FROM
        (SELECT p.name AS held FROM software_platforms spl JOIN platforms p ON p.slug = spl.platform_slug
         WHERE spl.software_slug = s.slug ORDER BY p.sort_order, p.name)) AS platform_names,
    (SELECT group_concat(held, ', ') FROM
        (SELECT i.name AS held FROM software_interfaces si JOIN interfaces i ON i.slug = si.interface_slug
         WHERE si.software_slug = s.slug ORDER BY i.sort_order, i.name)) AS interface_names,
    (SELECT group_concat(held, ', ') FROM
        (SELECT d.name AS held FROM software_devices sd JOIN devices d ON d.slug = sd.device_slug
         WHERE sd.software_slug = s.slug ORDER BY d.sort_order, d.name)) AS device_names,
    (SELECT group_concat(held, ', ') FROM
...
SELECT
    ...
    v.version,
    (SELECT group_concat(held, ', ') FROM
        (SELECT a.name AS held FROM file_architectures fa JOIN architectures a ON a.slug = fa.architecture_slug
         WHERE fa.file_id = f.id ORDER BY a.sort_order, a.name)) AS architecture,
    v.software_slug,
    s.name AS software_name,
    s.category AS category_slug,
    (SELECT group_concat(held, ', ') FROM
        (SELECT p.name AS held FROM file_platforms fp JOIN platforms p ON p.slug = fp.platform_slug
         WHERE fp.file_id = f.id ORDER BY p.sort_order, p.name)) AS platform_names,
    (SELECT group_concat(held, ', ') FROM
        (SELECT d.name AS held FROM file_devices fd JOIN devices d ON d.slug = fd.device_slug
         WHERE fd.file_id = f.id ORDER BY d.sort_order, d.name)) AS device_names,
    (SELECT group_concat(held, ', ') FROM
       (SELECT l.name AS held FROM file_languages fl JOIN languages l ON l.slug = fl.language_slug
        WHERE fl.file_id = f.id ORDER BY l.sort_order, l.name)) AS language_names
FROM files f
